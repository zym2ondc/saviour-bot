...
let started = false;
// Track consecutive failures per guild so a permanent block (e.g. 403)
// logs once instead of spamming every 2 minutes forever.
const failures = {};

function noteFailure(tiktok, err) {
  const f = failures[tiktok] || { count: 0, lastLogged: 0 };
  f.count += 1;
  failures[tiktok] = f;
  // Log the full error on the 1st failure, then at most once an hour
  if (f.count === 1 || Date.now() - f.lastLogged > 60 * 60 * 1000) {
    f.lastLogged = Date.now();
    console.warn(`📱 [media] check failed for @${tiktok} (${f.count}x):`, err.message);
  }
}

function noteSuccess(tiktok) {
  if (failures[tiktok]?.count > 1) console.log(`📱 [media] check for @${tiktok} recovered ✅`);
  delete failures[tiktok];
}
    ...
    try {
      ...
      for (const [guildId, cfg] of Object.entries(all)) {
        ...
        try {
          ...
          noteFailure(cfg?.tiktok || guildId, err);
        }
      }
    } catch (err) {
