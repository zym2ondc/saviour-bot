...
// yt-dlp binary bundled via the yt-dlp-exec package (actively maintained,
// handles YouTube bot-checks that break play-dl streaming).
function ytDlpBin() {
...
function ytDlpJson(url) {
  ...
  return new Promise((resolve, reject) => {
    execFile(ytDlpBin(), ['--dump-single-json', '--no-playlist', '--no-warnings', url], { timeout: 25_000, maxBuffer: 4 * 1024 * 1024, windowsHide: true }, (err, stdout, stderr) => {
...
async function playTrack(guild, track) {
  ...
  const proc = spawn(ytDlpBin(), args, { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
  s.proc = proc;
  let stderr = '';
