...
async function joinChannel(guild, voiceChannel) {
  ...
  connection.on('stateChange', (oldState, newState) => {
    try {
      const net = newState.networking ? ` networking=${newState.networking.state?.status}` : '';
...
async function resolveTracks(query, requestedBy) {
  // Direct YouTube video / playlist link, or search terms.
  if (isUrl(query)) {
    ...
    // SoundCloud track/playlist links
    try {
      const soType = await playdl.so_validate(query);
      if (soType === 'track') {
        const info = await playdl.soundcloud(query);
        return [{ title: info.name || 'Unknown title', url: info.url || query, duration: formatDur((info.durationInMs || 0) / 1000), thumbnail: info.thumbnail || null, requestedBy }];
      }
    } catch {}
    // Fallback: treat any other URL as a single playable stream (play-dl stream handles it)
