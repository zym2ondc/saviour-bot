...
async function fetchLatestVideo(username) {
  ...
  let res;
  try {
    res = await fetch(url, { headers: { 'User-Agent': 'SaviourBot/1.0' } });
  } catch (err) {
    throw new Error(`Couldn't reach TikTok lookup service (${err.message})`);
  }
  if (res.status === 403) {
    throw new Error(
      'TikTok auto-check is blocked from this server right now (TikWM is showing a bot-check to this IP). ' +
      'Use `/media post` with the video link instead — that works everywhere.'
    );
  }
